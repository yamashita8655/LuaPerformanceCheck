...
function LuaMain()
	...
	--LoadTest()
	--LoadFileTest()
	LuaUnityDebugLog("Before")
	TimeCheck()
	LuaUnityDebugLog("After")
end
	
function TimeCheck()
	-- フィボナッチ数列を求める処理で、時間計測するというのがあったので、拝借してみる
	LuaUnityDebugLog(fib(38))
end
...
function LoadFileTest()
	-- c:/～のパス
	local filePath = string.sub(StreamingDataPath, 9)
	local loadChunk = loadfile(filePath.."/LoadFileTest.lua")
	LuaUnityDebugLog(filePath)
	if loadChunk == nil then
		LuaUnityDebugLog("loadChunk is nil")
	else
		index = 0
		LuaUnityDebugLog("i = "..index)
		loadChunk()
		LuaUnityDebugLog("i = "..index)
	end

	-- もふねこで使ってた、ユーティリティのファイルを読んでみる
	local loadChunk2 = loadfile(filePath.."/LuaUtilityClass.lua")
	if loadChunk2 == nil then
		LuaUnityDebugLog("loadChunk2 is nil")
	else
		loadChunk2()
		local pos = Vector2.new(5, 10)
		LuaUnityDebugLog("x:"..pos.x.."y:"..pos.y)
	end
...
function MetaTableTest()
	...
	local useIndexTable = {}
	setmetatable(useIndexTable, {__index=glovalAccessTable})

	LuaUnityDebugLog("RESULT1:"..useIndexTable.glovalValue1)
	LuaUnityDebugLog("RESULT2:"..useIndexTable.glovalValue2)
	LuaUnityDebugLog("RESULT3:"..useIndexTable.glovalValue3[1])
end
